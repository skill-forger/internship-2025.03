...
func (r *repository) AddTag(postID, tagId int) error {
...
func (r *repository) GenerateSlug(title string) (string, error) {
	base := slug.Make(title)

	// Query duy nhất: đếm slug có cùng gốc
	var total int64
	r.db.
		Model(&model.Post{}).
	...
	if total == 0 {
		return base, nil // chưa ai dùng
	}
	return fmt.Sprintf("%s-%d", base, total+1), nil
}
